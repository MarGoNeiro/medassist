...
const fuse = new Fuse(drugs, { keys: ['inn', 'brand'], threshold: 0.3, minMatchCharLength: 2 })
...
function addRecent(drug) {
  ...
  filtered.unshift({ section: 'drugs', id: drug.id, label: drug.inn, ts: Date.now() })
...
function DrugCard({ drug, onBack }) {
  ...
  const severityConfig = {
    ...
    unknown: { color: '#64748B', bg: '#F1F5F9', icon: '⚪', label: 'Нет данных' },
  }
            ...
            {checkDrugs.length >= 2 && (
              <div style={{ marginTop: 16 }}>
                {interactionPairs.map((pair, i) => {
                  const cfg = pair.inter ? severityConfig[pair.inter.severity] : severityConfig.unknown
                  return (
                    <div key={i} className="interaction-card" style={{ background: cfg.bg, borderLeft: `4px solid ${cfg.color}` }}>
                      ...
                      {!pair.inter && <p className="interaction-desc" style={{ color: '#64748B' }}>Эта пара не найдена в базе — это не означает совместимость. Проверьте по справочнику РЛС или Видаль.</p>}
                    </div>
                  )
                })}
...
export default function Drugs({ initialId, pushBack, popBack }) {
  const [query, setQuery] = useState('')
  const [filter, setFilter] = useState('all')
  const [selected, setSelected] = useState(() => initialId ? drugs.find(d => d.id === initialId) || null : null)

  if (selected) return <DrugCard drug={selected} onBack={() => popBack?.()} />

  let results
  if (query.length >= 2) {
    const q = query.toLowerCase()
    const exact = drugs.filter(d =>
      d.inn.toLowerCase().includes(q) ||
      d.brand.toLowerCase().includes(q) ||
      d.group.toLowerCase().includes(q)
    )
    results = exact.length > 0 ? exact : fuse.search(query).map(r => r.item)
  } else {
    results = [...drugs].sort((a, b) => a.inn.localeCompare(b.inn, 'ru'))
  }
  ...
  return (
    <div className="page">
      <div className="section-hero section-hero--drugs">
        <p className="section-hero-label">Справочник</p>
        <h1 className="section-hero-title">Лекарственные средства</h1>
      </div>
      <div className="page-content">
        ...
        <div className="list-block" style={{ display: 'flex', flexWrap: 'wrap' }}>
          {results.map((drug, i) => (
            <button key={drug.id} className="list-item" onClick={() => selectDrug(drug)}
              style={{ width: '50%', boxSizing: 'border-box', borderRight: i % 2 === 0 ? '1px solid var(--color-border)' : 'none', flexDirection: 'column', alignItems: 'center', textAlign: 'center', justifyContent: 'center', position: 'relative' }}>
              {drug.znvlp && <span className="badge badge-green" style={{ position: 'absolute', top: 6, right: 6, fontSize: 9, padding: '2px 5px' }}>ЖНВЛП</span>}
              <div className="list-item-title">{drug.inn}</div>
              <div className="list-item-subtitle" style={{ marginTop: 2, whiteSpace: 'normal', overflow: 'visible', textOverflow: 'unset' }}>{drug.brand} · {drug.group}</div>
            </button>
          ))}
        </div>
      </div>
