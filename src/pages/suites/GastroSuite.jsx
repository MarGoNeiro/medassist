...
const CP_FIELDS = [
  { id: 'ascites',   label: 'Асцит',             opts: [{ v:1,l:'Нет [1]' }, { v:2,l:'Умеренный [2]' }, { v:3,l:'Напряжённый [3]' }] },
  { id: 'bili',      label: 'Билирубин',          opts: [{ v:1,l:'< 34 мкмоль/л [1]' }, { v:2,l:'34–51 мкмоль/л [2]' }, { v:3,l:'> 51 мкмоль/л [3]' }] },
  { id: 'albumin',   label: 'Альбумин',           opts: [{ v:1,l:'> 35 г/л [1]' }, { v:2,l:'28–35 г/л [2]' }, { v:3,l:'< 28 г/л [3]' }] },
  { id: 'pt',        label: 'ПВ / МНО',           opts: [{ v:1,l:'< 4 с / < 1.7 [1]' }, { v:2,l:'4–6 с / 1.7–2.3 [2]' }, { v:3,l:'> 6 с / > 2.3 [3]' }] },
  { id: 'encephalo', label: 'Энцефалопатия',      opts: [{ v:1,l:'Нет [1]' }, { v:2,l:'I–II степень [2]' }, { v:3,l:'III–IV степень [3]' }] },
]

function cpResult(score) {
  if (score <= 6) return { cls: 'A', badge: 'badge-green',  advice: '1-летняя выживаемость ≈ 100%. Плановые вмешательства возможны.' }
  if (score <= 9) return { cls: 'B', badge: 'badge-yellow', advice: '1-летняя выживаемость ≈ 80%. Оценка на трансплантацию.' }
  return               { cls: 'C', badge: 'badge-red',    advice: '1-летняя выживаемость ≈ 45%. Приоритет для трансплантации. MELD.' }
}

const CP_CLASS_LABEL = { A: 'Компенсирован', B: 'Субкомпенсирован', C: 'Декомпенсирован' }
...
const ROME_IV = [
  { dx: 'СРК',               criteria: 'Боль в животе ≥ 1 р/нед × 3 мес. + ≥ 2 из: связь со стулом, изм. частоты или формы стула.' },
  { dx: 'ГЭРБ',              criteria: 'Изжога / регургитация ≥ 2 р/нед. ± эзофагит при ЭГДС. Отвечает на ИПП.' },
  { dx: 'Функц. запор',      criteria: 'Натуживание / ком / твёрдый стул / неполное опорожнение / < 3 стулов/нед. в ≥ 25% дефекаций.' },
  ...
  return (
    <div className="suite">
      ...
      <div className="gastro-main-col">
        ...
        <div className="suite-card gastro-cp-card">
          <div className="suite-card-title">🔬 Child-Pugh — тяжесть цирроза печени</div>
          <div className="suite-grid gastro-cp-grid">
            {CP_FIELDS.map(f => (
          ...
          <div className="suite-result-banner scorad-result">
            ...
            <div className="scorad-verdict">
              <span className={`suite-risk-badge ${cpRes.badge}`}>{CP_CLASS_LABEL[cpRes.cls]}</span>
              <div className="suite-advice">{cpRes.advice}</div>
            </div>
          </div>
        </div>

        {/* Blatchford — правая колонка */}
        <div className="suite-card gastro-blatchford-card">
          <div className="suite-card-title">🩸 Критерии высокого риска ЖКК (Blatchford)</div>
          <p className="gastro-hint">Любой признак — показание к экстренной ЭГДС и госпитализации:</p>
          <div className="suite-criteria-2col">
            {BLATCHFORD.map((item, i) => (
              <div key={i} className="suite-screening-item">
                <div className="suite-screening-dot gastro-dot" />
                <div className="suite-screening-desc gastro-criteria-text">{item}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Рим IV — полная ширина */}
      <div className="suite-card">
        <div className="suite-card-title">📋 Рим IV — диагностические критерии</div>
        <div className="rome-two-col">
          {ROME_IV.map((r, i) => (
            <div key={i} className="rome-item">
              <div className="rome-dx">{r.dx}</div>
              <div className="rome-criteria">{r.criteria}</div>
            </div>
          ))}
        </div>
      </div>
